...
P3 <- tibble(read.delim("#17coloc2Log_allz_corr.txt")) 
P4 <- tibble(read.delim("set4coloc2Log_allz.txt")) 

#Import set2 data (tr2)
C1_tr2 <- tibble(read.delim("set2_C1_coloc2Log_allZ.txt")) 
C2_tr2 <- tibble(read.delim("set2_C2_coloc2Log_allZ.txt")) 
P1_tr2 <- tibble(read.delim("set2_P2_coloc2Log_allZ.txt")) 
P2_tr2 <- tibble(read.delim("set2_P2_coloc2Log_allZ.txt")) 
P3_tr2 <- tibble(read.delim("set2_P3_coloc2Log_allZ.txt")) 
P4_tr2 <- tibble(read.delim("set2_P4_coloc2Log_allz.txt")) 
...
write_csv(P4_merge_corr, "Pearson_values_P4_tr2.csv")
...
merge_all <- bind_rows(C1_merge_corr, C2_merge_corr, P1_merge_corr, P2_merge_corr,
                       ...
                       P1_tr2_merge_corr, P2_tr2_merge_corr, P3_tr2_merge_corr,
                       P4_tr2_merge_corr)

# Outliers for C2, P3, and P4 determined via boxplot
# Outliers were then manually inspected. 
# R values < 0.3 for C2 and P3 found to be due to abnormally high signal 
# intensity in those cells skewing results. 
# Low R values for P4 appear to be correct and will be included. 
# Remove C2 and P3 outliers
merge_all_no_outliers <- merge_all %>%
  filter(., as.numeric(Pearson_no_threshold) > 0.3 | sample == "P4")
...
# Each dot corresponds to one cell.
# Minimum of 32 cells used in analysis (R710G - poor growth) with other samples having
# significantly more cells
merge_all_no_outliers %>%
  ...
  scale_color_manual(values = c("C1" = "#5d57a4", "C2" = "#8f8cc3",
                               "P1" = "#157c5e", "P2" = "#b24e02",
                               "P3" = "#c08f02", "P4" = "#ae1462")) +
  labs(title = "Drp1 vs. TOM20 colocalization", x = "sample",
       y = "Pearson's R") +
...
ggsave("20220531_Pearson_Drp1_TOM20_no_outliers.pdf", height = 20, width = 30, dpi = 300, units = "cm") 
ggsave("20220531_Pearson_Drp1_TOM20_no_outliers.png", height = 20, width = 30, dpi = 300, units = "cm") 

# Determine if differences in average DRP1-TOM20 colocalization values between
# samples is significant
anova_merge_all <- aov(Pearson_no_threshold ~ sample, data = merge_all_no_outliers)
summary(anova_merge_all)

# Perform post-hoc testing via Tukey
